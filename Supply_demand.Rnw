...
E_p_canopy <- matrix(0,0,nrow=1000, ncol=5) # this is the supply function
...
for(i in 1:length(predawn_soil_mat_pot))
  {
    cond_max_slope_sperry[i] <-  k_p_canopy(predawn_soil_mat_pot[i]) 
...
for(j in 1:5)
{
  ...
  for(i in 1:1000)
  {
   ...
   non_regulated_Gs[i,j] <- non_regulated_trans[i,j]/D # E = G*VPD ---- G = E/VPD (VPD=1, 0.001 transforms to MPa)
  ...
  # 
   ffx <- integrate(k_p_canopy, predawn_soil_mat_pot[j], predawn_soil_mat_pot[j] + reg_leaf_psi[i,j] )
   regulated_trans[i,j] <- pmax(0, ffx$value)
   regulated_Gs[i,j] <- regulated_trans[i,j]/D # E = G*VPD ---- G = E/VPD (VPD=1, 0.001 transforms to MPa)
...
plot(psi_canopy, (1 - sperry_cond(psi_canopy)/sperry_cond(0))*100, type="l", ylab="% loss of conductance", xlab="Sap pressure (-MPa)", cex.lab=2.0, cex.axis=2.0,lwd=2.5)
