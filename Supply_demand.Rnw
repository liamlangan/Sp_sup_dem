...
for(i in 1:5)
  {
    ...
    psi_demand_met_at_sperry[i] <- psi_canopy[demand_met_at_sperry[i]] # the matric potential where demand is met
    demand_met_at_slope_sperry[i] <- sperry_cond(psi_demand_met_at_sperry[i]) # slope of tangent to suppy curve where demand is met
