...
\hlstd{psi_canopy} \hlkwb{<-} \hlkwd{seq}\hlstd{(}\hlnum{0.0}\hlstd{,} \hlnum{8}\hlstd{,} \hlkwc{length}\hlstd{=}\hlnum{1000}\hlstd{)}
\hlstd{predawn_soil_mat_pot} \hlkwb{<-} \hlkwd{seq}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{3}\hlstd{,} \hlkwc{length}\hlstd{=}\hlnum{5}\hlstd{)} \hlcom{# this assumes initial plant matric potential is the same as the soil matric potential}
...
\hlstd{\{}
  ...
  \hlstd{\{}
   ...
   \hlstd{ffx} \hlkwb{<-} \hlkwd{integrate}\hlstd{(sperry_cond, predawn_soil_mat_pot[j], psi_1[i,j] )}
   \hlstd{non_regulated_trans[i,j]} \hlkwb{<-} \hlkwd{pmax}\hlstd{(}\hlnum{0}\hlstd{, ffx}\hlopt{$}\hlstd{value)}
   \hlstd{non_regulated_Gs[i,j]} \hlkwb{<-} \hlstd{non_regulated_trans[i,j]}\hlopt{/}\hlstd{VPD} \hlcom{# E = G*VPD ---- G = E/VPD (VPD=1, 0.001 transforms to MPa)}
  ...
  \hlcom{# }
   \hlstd{ffx} \hlkwb{<-} \hlkwd{integrate}\hlstd{(sperry_cond, predawn_soil_mat_pot[j], predawn_soil_mat_pot[j]} \hlopt{+} \hlstd{reg_leaf_psi[i,j] )}
   \hlstd{regulated_trans[i,j]} \hlkwb{<-} \hlkwd{pmax}\hlstd{(}\hlnum{0}\hlstd{, ffx}\hlopt{$}\hlstd{value)}
   \hlstd{regulated_Gs[i,j]} \hlkwb{<-} \hlstd{regulated_trans[i,j]}\hlopt{/}\hlstd{VPD} \hlcom{# E = G*VPD ---- G = E/VPD (VPD=1, 0.001 transforms to MPa)}
